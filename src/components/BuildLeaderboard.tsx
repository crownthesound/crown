...
import { Upload, CheckCircle, X, Move, Trophy, Link as LinkIcon, Plus, Trash2, Loader2, CircleDot } from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';

const DEFAULT_PRIZE_TITLES = ['First Place', 'Second Place', 'Third Place', 'Fourth Place', 'Fifth Place'];
...
const STEPS = [
  ...
  { id: 'resources', label: 'Resources', icon: CircleDot },
...
export function BuildLeaderboard() {
  ...
  const [config, setConfig] = useState({
    ...
    prize_tier: 'non-monetary' as const,
    total_prize: 5000,
    music_category: '',
  ...
  const fetchContestData = async () => {
    try {
      ...
      if (!data) {
        toast.error('Contest not found or not in draft status');
        navigate('/contests');
        return;
      }
      ...
      setConfig({
        ...
        end_date: data.end_date ? new Date(data.end_date).toISOString().slice(0, 16) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16),
        num_winners: data.num_winners || 5,
        prize_per_winner: data.prize_per_winner || 2000,
        prize_tier: (data.prize_tier as 'non-monetary' | 'monetary') || 'non-monetary',
        total_prize: data.total_prize || 5000,
        music_category: data.music_category || '',
        resources: data.resources || [],
        prize_titles: data.prize_titles || DEFAULT_PRIZE_TITLES.map((title, index) => ({
          rank: index + 1,
          title
        })),
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="p-4">
            {step === 'get-started' && (
              <div>
                <h2 className="text-xl font-bold mb-6">Customize your contest</h2>
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setConfig(prev => ({ ...prev, music_category: e.target.value }))}
                      className="w-full px-4 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-400"
                      required
                    >
                        ...
                        <div className="relative">
                          ...
                          <div className="absolute bottom-4 right-4 bg-white rounded-lg shadow-lg p-2">
                            <div className="grid grid-cols-3 gap-1">
                              ...
                              <button 
                                className="p-2 hover:bg-gray-100 rounded-md"
                                onClick={() => handleImagePositionChange('down')}
                              >
                                <Move className="h-4 w-4 rotate-90" />
                              </button>
                              <div className="p-1" />
                        ...
                        <label className="cursor-pointer block p-6">
                          <div className="text-center">
                            <div className="flex flex-col items-center">
                              ...
                              <p className="text-gray-600">Drag, drop or browse thumbnail</p>
            ...
            {step === 'resources' && (
              <div>
                <h2 className="text-xl font-bold mb-6">Add Resources</h2>
                <div className="space-y-6">
                  ...
                  {config.resources.length > 0 && (
                    <div className="space-y-4">
                      <h3 className="text-lg font-semibold">Added Resources</h3>
                  ...
                  {config.resources.length === 0 && (
                    <div className="text-center text-gray-500 py-8">
                      No resources added yet. Add some resources to help participants understand the contest better.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                  <div
                    ...
                    onClick={() => setConfig(prev => ({ ...prev, prize_tier: 'non-monetary' }))}
                  >
                    <div className="text-lg font-semibold mb-2">Title Based</div>
                    <div className="text-2xl font-bold mb-2">Title Rewards</div>
                    <p className="text-sm text-gray-600">Award custom titles to winners</p>
                  </div>
                  ...
                  <div
                    ...
                    onClick={() => setConfig(prev => ({ ...prev, prize_tier: 'monetary' }))}
                  >
                    <div className="text-lg font-semibold mb-2">Money Based</div>
                    <div className="text-2xl font-bold mb-2">Custom Prize Pool</div>
                    <p className="text-sm text-gray-600">Set custom monetary prizes for each winner</p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <button
                        onClick={() => handleUpdateNumWinners(Math.max(1, config.num_winners - 1))}
                        className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200"
                        disabled={config.num_winners <= 1}
                      >
                        -
                      </button>
                      <span className="text-lg font-medium w-12 text-center">{config.num_winners}</span>
                      <button
                        onClick={() => handleUpdateNumWinners(Math.min(10, config.num_winners + 1))}
                        className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200"
                    ...
                    <p className="text-sm text-gray-500 mt-1">Minimum 1, maximum 10 winners</p>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                      ...
                      {config.prize_tier === 'non-monetary' ? 'Prize Titles' : 'Prize Distribution'}
                    </h3>
                              ...
                              <div className="flex-1 flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  onChange={(e) => setConfig(prev => ({
                                    ...prev,
                                    prize_per_winner: parseInt(e.target.value) || 0
                                  }))}
                                  className="flex-1 px-4 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-400"
